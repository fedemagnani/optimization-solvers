...
use optimization_solvers::{FuncEvalMultivariate, LineSearchSolver, MoreThuente, Tracer, SR1B};
...
fn main() {
    ...
    // Convex function: f(x,y) = x^2 + 3y^2 + xy
    ...
    let f_and_g = |x: &DVector<f64>| -> FuncEvalMultivariate {
        ...
        // Function value
        let f = x1.powi(2) + 3.0 * x2.powi(2) + x1 * x2;
    ...
    let mut solver = SR1B::new(tol, x0.clone(), lower_bound.clone(), upper_bound.clone());

    // Running the solver
    let max_iter_solver = 100;
    let max_iter_line_search = 20;

    println!("=== SR1B (Bounded SR1) Example ===");
    println!("Objective: f(x,y) = x^2 + 3y^2 + xy (convex quadratic)");
