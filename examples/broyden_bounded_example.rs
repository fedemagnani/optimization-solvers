...
use optimization_solvers::{BroydenB, FuncEvalMultivariate, LineSearchSolver, MoreThuente, Tracer};
...
fn main() {
    ...
    let mut solver = BroydenB::new(tol, x0.clone(), lower_bound.clone(), upper_bound.clone());

    // Running the solver
    let max_iter_solver = 100;
    let max_iter_line_search = 20;

    println!("=== BroydenB (Bounded Broyden) Example ===");
    ...
    match solver.minimize(
        ...
        Ok(()) => {
            ...
            // Show some properties of BroydenB
            println!("BroydenB properties:");
            println!("  - Broyden's method with box constraints");
            println!("  - Uses rank-1 Hessian updates");
