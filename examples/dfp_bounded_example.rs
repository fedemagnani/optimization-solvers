...
use optimization_solvers::{FuncEvalMultivariate, LineSearchSolver, MoreThuente, Tracer, DFPB};
...
fn main() {
    ...
    // Convex function: f(x,y) = x^2 + 4y^2 + xy
    ...
    let f_and_g = |x: &DVector<f64>| -> FuncEvalMultivariate {
        ...
        // Function value
        let f = x1.powi(2) + 4.0 * x2.powi(2) + x1 * x2;
    ...
    let upper_bound = DVector::from_vec(vec![1.5, 1.5]); // x <= 1.5, y <= 1.5
    let mut solver = DFPB::new(tol, x0.clone(), lower_bound.clone(), upper_bound.clone());

    // Running the solver
    let max_iter_solver = 100;
    let max_iter_line_search = 20;

    println!("=== DFPB (Bounded DFP) Example ===");
    println!("Objective: f(x,y) = x^2 + 4y^2 + xy (convex quadratic)");
    println!("Unconstrained minimum: solution of ∇f(x) = 0");
    println!("Constraints: 0 <= x <= 1.5, 0 <= y <= 1.5");
    ...
    match solver.minimize(
        ...
        Ok(()) => {
            ...
            // Show some properties of DFPB
            println!("DFPB properties:");
            println!("  - DFP with box constraints");
            println!("  - Updates inverse Hessian approximation");
            println!("  - One of the first quasi-Newton methods");
