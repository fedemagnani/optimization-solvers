...
fn main() {
    ...
    // for _ in 0..pools_m {
    //     let r0 = 1e6 * rng.gen::<f64>();
    //     let r1 = 1e6 * rng.gen::<f64>();
    ...
    let upper_bound = DVector::from_vec(vec![Floating::INFINITY; assets_n]);

    // Setting up the line search
    // let mut ls = BackTrackingB::new(1e-4, 0.25, lower_bound.clone(), upper_bound.clone());
    // let mut ls = GLLQuadratic::new(1e-4, 1).with_sigmas(0.1, 0.9);
    let mut ls = MoreThuente::default();

    // Setting up the solver
    // let mut gd =
    //     SpectralProjectedGradient::new(grad_tol, x0, &mut f_and_g, lower_bound, upper_bound);
    // let mut gd = ProjectedGradientDescent::new(grad_tol, x0, lower_bound, upper_bound);
    let mut gd = BFGSB::new(grad_tol, x0, lower_bound, upper_bound);
