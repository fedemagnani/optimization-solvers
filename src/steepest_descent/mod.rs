...
pub struct SteepestDescent<T, S> {
    ...
    spg: Option<DVector<Floating>>, // 12.14 from [Neculai Andrei, 2022]
}
    ...
    S: ComputeDirection,
{
    ...
    fn has_converged(&self, eval: &FuncEvalMultivariate) -> bool {
        // we verify that the squared euclidean norm of the gradient is below the tolerance. If the spectral projected gradient is available, then it means that we are in a constrained optimization setting and we verify if it is zero since this is equivalent to first order conditions of optimility in the setting of optimization with simple bounds (Theorem 12.3 from [Neculai Andrei, 2022])

        let grad = match &self.spg {
            Some(spg) => spg,
            None => eval.g(),
        };
        grad.dot(&grad) < self.grad_tol

        // // we project the gradient if bounds are present
        // match (self.lower_bound.as_ref(), self.upper_bound.as_ref()) {
        //     (Some(lower), Some(upper)) => {
        //         let x = self.xk();
        //         let mut proj_mut = eval.g().clone();
        //         for (i, (x, g)) in x.iter().zip(proj_mut.iter_mut()).enumerate() {
        //             if (x == &lower[i] && g > &mut 0.0) || (x == &upper[i] && g < &mut 0.0) {
        //                 *g = 0.0;
        //             }
        //         }
        //         debug!(target: "steepest_descent", "x_k: {:?}, Gradient: {:?}, Projected gradient: {:?}", self.xk(), eval.g(), proj_mut);
        //         proj_mut.iter().fold(0.0f64, |acc, x| acc.max(x.abs())) < self.grad_tol
        //     }
        //     (Some(lower), None) => {
        //         let x = self.xk();
        //         let mut proj_mut = eval.g().clone();
        //         for (i, (x, g)) in x.iter().zip(proj_mut.iter_mut()).enumerate() {
        //             if x == &lower[i] && g > &mut 0.0 {
        //                 *g = 0.0;
        //             }
        //         }
        //         debug!(target: "steepest_descent", "x_k: {:?}, Gradient: {:?}, Projected gradient: {:?}", self.xk(), eval.g(), proj_mut);
        ...
        //         proj_mut.iter().fold(0.0f64, |acc, x| acc.max(x.abs())) < self.grad_tol
        //     }
        //     (None, None) => eval.g().dot(eval.g()) < self.grad_tol,
        // }
    }
    ...
    fn update_next_iterate(
        ...
        // we project the next iterate and compute the spectral projected gradient if bounds are present;
        let next_iterate = match (&self.lower_bound, &self.upper_bound) {
            (Some(lower), Some(upper)) => {
                let next_proj = next_iterate.box_projection(lower, upper);
                self.spg = Some(&next_proj - self.xk());
                next_proj
            }
            (Some(lower), None) => {
